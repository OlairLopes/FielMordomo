    ...
    with st.expander("Novo cadastro", expanded=False):
        with st.form("form_novo_cad", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar", type="primary"):
                ...
                if cpf_limpo and cpf_existe(slug, cpf_limpo):
                    erros.append("CPF ja cadastrado. Verifique se este cadastro ja existe.")
                if erros:
    ...
    with st.expander("Editar ou excluir cadastro", expanded=False):
        ...
        with c1:
            ...
            if solicitar_autorizacao("salvar_cad", "editar"):
                ...
                erros = c.validar()
                cpf_limpo = "".join(d for d in cpf_edit if d.isdigit())
                if cpf_limpo and cpf_existe(slug, cpf_limpo, id_excluir=id_sel):
                    erros.append("CPF ja cadastrado em outro registro.")
                if erros:
                    for e in erros: st.error(e)
                else:
